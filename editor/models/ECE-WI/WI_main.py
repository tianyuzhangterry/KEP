...
from .WI_hparams import WIHyperParams
...
def apply_WI_to_model(
    model: AutoModelForCausalLM,
    tok: AutoTokenizer,
    requests: List[Dict],
    hparams: WIHyperParams,
    cache_template: Optional[str] = None,
    cache_c = None,
) -> Dict[str, Tuple[torch.Tensor]]:
    """
    Executes the WI update algorithm for the specified update at the specified layer
    ...
    for i, layer in enumerate(hparams.layers):
        ...
        cumulative_token_scores = torch.sum(torch.abs(W_out), dim=0)  
        ...
        neuron_indices = cumulative_sorted_indices[:torch.where(torch.cumsum(cumulative_sorted_scores, dim=0) <= hparams.neuron_threshold*cumulative_token_scores.sum())[0][-1] + 2].cpu().tolist()
        ...
        #W_out, targets = W_out.double(), targets.double()
        layer_ks, targets = (
            layer_ks.double(),
            targets.double(),
        )
...
def upd_matrix_match_shape(matrix: torch.Tensor, shape: torch.Size) -> torch.Tensor:
    ...
    if matrix.shape == shape:
        ...
        raise ValueError(
            "Update matrix computed by WI does not match original weight shape. "
